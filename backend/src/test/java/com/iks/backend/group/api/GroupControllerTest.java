...
class GroupControllerTest {
    ...
    void createGroupReturnsCreatedResponseWithLocation() {
        ...
        ResponseEntity<GroupResponse> response = controller.createGroup(
            new CreateGroupRequest("Weekend Hikers", "desc")
        );
    ...
    void addMemberDelegatesToService() {
        ResponseEntity<Void> response = controller.addMember(
            "group-1",
            new AddGroupMemberRequest("user-1", null)
        );
    ...
    void listMembersMapsToApiResponse() {
        when(groupService.listGroupMembers("group-1")).thenReturn(
            List.of(
