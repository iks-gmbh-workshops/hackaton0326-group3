...
class ActivityNotificationServiceTest {
    ...
    @BeforeEach
    void setUp() {
        notificationService = new ActivityNotificationService(
            emailService,
            keycloakService,
            "http://frontend.local/"
        );
    ...
    void notifyActivityCreatedFiltersRecipientsAndBuildsEscapedHtmlBody() {
        ...
        verify(emailService).sendEmailToMany(recipientsCaptor.capture(), subjectCaptor.capture(), bodyCaptor.capture());
    ...
    void notifyAttendanceChangedUsesStatusLabelAndVerifiedRecipients() {
        ...
        notificationService.notifyAttendanceChanged(activity, group, "Jane Doe", AttendanceStatus.MAYBE);
    ...
    private static UserRepresentation member(String id, String email, String username, boolean verified) {
