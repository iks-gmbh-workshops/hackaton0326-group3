...
public class ActivityService {
  ...
  public Activity createActivity(
    ...
    Activity activity =
        new Activity(groupId, title, normalizedDescription, scheduledAt, normalizedLocation);
    Activity saved = activityRepository.saveAndFlush(activity);
    userService.createActivityInviteNotifications(
        group.getId(), group.getName(), saved.getId(), saved.getTitle(), currentUserId);
  ...
  private static String normalizeDescription(String rawDescription) {
    ...
    if (description.length() > 2000) {
      throw new InvalidActivityRequestException(
          "Activity description must be at most 2000 characters");
    }
    return description;
  }
  ...
  private static LocalDateTime parseDateTime(String date, String time) {
    ...
    try {
      ...
      throw new InvalidActivityRequestException(
          "Invalid date or time format. Please check that the date exists (e.g., February only has 28/29 days)",
          e);
    }
  ...
  @Transactional
  public ActivityAttendance respondToActivity(
      String activityId, String groupId, String statusStr, String userName, String userEmail) {
    Activity activity = getActivity(activityId);
    groupService.getGroup(groupId);

    if (activity.getScheduledAt().isBefore(LocalDateTime.now())) {
      throw new InvalidActivityRequestException(
          "Cannot change attendance after the activity has started");
    }
  ...
  public List<ActivityWithStatus> listMyAcceptedActivities() {
    String currentUserId = getCurrentUserId();
    List<ActivityAttendance> acceptedAttendances =
        attendanceRepository.findAcceptedByUserId(currentUserId);

    List<String> activityIds =
        acceptedAttendances.stream().map(ActivityAttendance::getActivityId).toList();
        ...
        .flatMap(
            attendance ->
                activities.stream()
                    ...
                    .map(
                        activity ->
                            new ActivityWithStatus(activity, attendance.getStatus().name())))
        .toList();
  }
  ...
  private String getCurrentUserId() {
    ...
    if (principal instanceof Jwt jwt) {
      ...
      if (userId == null || userId.isBlank()) {
        throw new InvalidActivityRequestException(
            "Unable to determine user ID from authentication token");
      }
      return userId;
    }
