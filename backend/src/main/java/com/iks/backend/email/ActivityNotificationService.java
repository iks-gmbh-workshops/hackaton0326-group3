...
public class ActivityNotificationService {
  ...
  public void notifyActivityCreated(Activity activity, AppGroup group, String creatorUserId) {
    ...
    String htmlBody =
        "<h2>New Activity Created</h2>"
            + "<p>A new activity has been created in <strong>"
            + escapeHtml(group.getName())
            + "</strong>.</p>"
            + "<table style=\"border-collapse:collapse;margin:16px 0\">"
            + "<tr><td style=\"padding:4px 12px 4px 0;font-weight:bold\">Title</td><td>"
            + escapeHtml(activity.getTitle())
            + "</td></tr>"
            + "<tr><td style=\"padding:4px 12px 4px 0;font-weight:bold\">Date</td><td>"
            + dateStr
            + "</td></tr>"
            + "<tr><td style=\"padding:4px 12px 4px 0;font-weight:bold\">Time</td><td>"
            + timeStr
            + "</td></tr>"
            + (activity.getLocation() != null
                ? "<tr><td style=\"padding:4px 12px 4px 0;font-weight:bold\">Location</td><td>"
                    + escapeHtml(activity.getLocation())
                    + "</td></tr>"
                : "")
            + (activity.getDescription() != null
                ? "<tr><td style=\"padding:4px 12px 4px 0;font-weight:bold\">Description</td><td>"
                    + escapeHtml(activity.getDescription())
                    + "</td></tr>"
                : "")
            + "</table>"
            + "<p><a href=\""
            + escapeHtml(activityLink)
            + "\">Open activity</a> to view details and respond.</p>";
  ...
  public void notifyAttendanceChanged(
      Activity activity, AppGroup group, String userName, AttendanceStatus status) {
    List<UserRepresentation> members = keycloakService.listGroupMembers(group.getId());
