...
public class KeycloakAdminClientService implements KeycloakService {
      ...
      try {
        ...
        if (status != 201) {
          throw new KeycloakServiceException(
              "Failed to create group in Keycloak, status: " + status);
        }
  ...
  public void removeUserFromGroup(String userId, String groupId) {
    try (Keycloak keycloak = buildAdminClient()) {
      ...
      throw new KeycloakServiceException(
          "Failed to remove user from Keycloak group", runtimeException);
    }
  }
  ...
  public List<UserRepresentation> listGroupMembers(String groupId) {
    try (Keycloak keycloak = buildAdminClient()) {
      ...
      return members.stream().filter(user -> user.isEnabled() == null || user.isEnabled()).toList();
    } catch (GroupNotFoundException missingGroup) {
      throw missingGroup;
    } catch (RuntimeException runtimeException) {
      throw new KeycloakServiceException(
          "Failed to list group members from Keycloak", runtimeException);
    }
  }
  ...
  public List<String> listUserGroupIds(String userId) {
    try (Keycloak keycloak = buildAdminClient()) {
      ...
      throw new KeycloakServiceException(
          "Failed to list user groups from Keycloak", runtimeException);
    }
  }
  ...
  public Optional<UserRepresentation> findUserByEmail(String email) {
    try (Keycloak keycloak = buildAdminClient()) {
      ...
      throw new KeycloakServiceException(
          "Failed to search user by email in Keycloak", runtimeException);
    }
  }
  ...
  public void sendRequiredActionsEmail(
      String userId, String redirectUri, List<String> requiredActions) {
    try (Keycloak keycloak = buildAdminClient()) {
      RealmResource realm = keycloak.realm(properties.getRealm());
      UserResource userResource = realm.users().get(userId);
      try {
      ...
      throw new KeycloakServiceException(
          "Failed to send required-actions email via Keycloak", runtimeException);
    }
  }
